...
namespace Heimdall.Infrastracture.Database.Command.GlucoseRecordsCommands
{
    public class CreateGlucoseRecordCommand: ICommandHandler<CreateGlucoseRecordRequest, CreateGlucoseRecordResponse>
    {
        ...
        {
            var glucoseRecordId =
                await _unitOfWork.GlucoseRecordRepository.InsertGlucoseRecord(_mapper.Map<GlucoseRecord>(request));
            return _mapper.Map<CreateGlucoseRecordResponse>(
                await _unitOfWork.GlucoseRecordRepository.GetGlucoseRecordByID(glucoseRecordId));
        }
    }
}
